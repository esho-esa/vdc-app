  ...
  if (accountSid && authToken && fromNumber) {
    try {
      ...
      const result = await client.messages.create({
        body: message,
        from: fromNumber,
        to: `whatsapp:${phone.startsWith('+') ? phone : '+91' + phone}`, // Default to India code if missing
      });

      console.log(`[WhatsApp Reminder] Real Message sent to: ${phone}, SID: ${result.sid}`);
      ...
      console.error(`[WhatsApp Reminder] Twilio Error:`, error.message);
      return {
        success: false,
        error: error.message,
      };
    ...
    // Fallback Mock Logic
    console.log(`[WhatsApp Reminder MOCK] To: ${phone}`);
    console.log(`[WhatsApp Reminder MOCK] Message: ${message}`);
...
export async function sendFollowUpReminder({ patientPhone, patientName, clinicName, treatmentType }) {
  ...
  return sendWhatsAppReminder(patientPhone, message);
}
