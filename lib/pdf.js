 ...
 * @returns {Promise<Buffer>} - The PDF as a buffer
 */
export async function generateBillingReportPDF({ patient, treatments, totalPaid, outstandingBalance, startDate, endDate, dentist, settings }) {
  const clinicSettings = settings || {
    clinic_name: 'Victoria Dental Care',
    tagline: 'Premium Dental Solutions',
    address: 'No 1/334 Injambakkam, Opp to Suga Jeeva Peralayam, Ammathi, Perumal Koil St, Chennai',
  ...
  const totalPayments = typeof totalPaid === 'number' ? totalPaid : totalBilled;
  const outstandingAmt = typeof outstandingBalance === 'number' ? outstandingBalance : 0;
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      drawSummaryBox(386, summaryY, boxW, boxH, 'OUTSTANDING BALANCE', `₹${outstandingAmt.toLocaleString('en-IN')}`, outstandingAmt > 0 ? '#ff3b30' : textColor);
...
export async function generatePaymentReceiptPDF({ payment, patient, remainingBalance, settings }) {
  const clinicSettings = settings || {
    clinic_name: 'Victoria Dental Care',
    tagline: 'Premium Dental Solutions',
    address: 'No 1/334 Injambakkam, Opp to Suga Jeeva Peralayam, Ammathi, Perumal Koil St, Chennai',
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Left: Clinic Info
      doc.fontSize(16).font('Helvetica-Bold').fillColor(textColor)
        .text(clinicSettings.clinic_name, 50, 40);
      doc.fontSize(9).font('Helvetica').fillColor(secondaryColor)
        .text(clinicSettings.tagline, 50, 58);
      doc.fontSize(8).fillColor(textColor);
      doc.text(`Phone: ${clinicSettings.phone}`, 50, 72, { width: 240 });
      doc.text(`Email: ${clinicSettings.email}`, 50, 83, { width: 240 });
      doc.text(`Address: ${clinicSettings.address}`, 50, 94, { width: 240 });
      ...
      doc.fontSize(18).font('Helvetica-Bold').fillColor(primaryColor)
        .text('PAYMENT RECEIPT', 300, 40, { width: 245, align: 'right' });
      doc.fontSize(9).font('Helvetica').fillColor(textColor);
      doc.text(`Receipt No: ${receiptNo}`, 300, 65, { width: 245, align: 'right' });
      doc.text(`Date: ${payment.payment_date}`, 300, 78, { width: 245, align: 'right' });

      // Separator line
      doc.save();
      doc.rect(50, 125, 495, 1.5).fill(textColor);
      doc.restore();

      // Patient Details
      let currentY = 145;
      doc.fontSize(11).font('Helvetica-Bold').fillColor(primaryColor).text('PATIENT INFORMATION', 50, currentY);
      currentY += 18;

      doc.fontSize(9).font('Helvetica').fillColor(textColor);
      doc.text(`Name:`, 50, currentY, { width: 80, align: 'left' });
      doc.font('Helvetica-Bold').text(`${patient.name}`, 130, currentY, { width: 160 });
      doc.font('Helvetica');
      doc.text(`Age:`, 300, currentY, { width: 80 });
      doc.text(`${patient.age || 'N/A'}`, 380, currentY, { width: 165 });
      currentY += 15;

      doc.text(`Phone:`, 50, currentY, { width: 80 });
      doc.text(`${patient.phone || 'N/A'}`, 130, currentY, { width: 160 });
      doc.text(`Email:`, 300, currentY, { width: 80 });
      doc.text(`${patient.email || 'N/A'}`, 380, currentY, { width: 165 });
      currentY += 15;

      if (patient.address) {
        doc.text(`Address:`, 50, currentY, { width: 80 });
        doc.text(`${patient.address}`, 130, currentY, { width: 350 });
        currentY += 25;
      } else {
        currentY += 10;
      }
      ...
      // Separator line
      doc.save();
      doc.rect(50, currentY, 495, 1).fill('#e9ecef');
      doc.restore();
      currentY += 15;

      // Payment Details Box
      doc.fontSize(11).font('Helvetica-Bold').fillColor(primaryColor).text('PAYMENT DETAILS', 50, currentY);
      currentY += 20;

      doc.save();
      doc.roundedRect(50, currentY, 495, 75, 6).fill('#f8f9fa');
      doc.roundedRect(50, currentY, 495, 75, 6).lineWidth(0.5).strokeColor('#e9ecef').stroke();
      doc.restore();

      doc.fontSize(10).font('Helvetica-Bold').fillColor(secondaryColor).text('AMOUNT PAID', 70, currentY + 15);
      doc.fontSize(22).font('Helvetica-Bold').fillColor(primaryColor).text(`₹${parseFloat(payment.amount).toLocaleString('en-IN')}`, 70, currentY + 28);

      doc.fontSize(9).font('Helvetica').fillColor(textColor);
      doc.text(`Payment Method:`, 280, currentY + 15, { width: 100 });
      doc.font('Helvetica-Bold').text(`${payment.payment_method}`, 380, currentY + 15, { width: 150 });
      doc.font('Helvetica');
      doc.text(`Reference No:`, 280, currentY + 30, { width: 100 });
      doc.font('Helvetica-Bold').text(`${payment.reference_number || 'N/A'}`, 380, currentY + 30, { width: 150 });
      doc.font('Helvetica');
      doc.text(`Remaining Balance:`, 280, currentY + 45, { width: 100 });
      doc.font('Helvetica-Bold').fillColor(remainingBalance > 0 ? '#ff3b30' : '#34c759').text(`₹${parseFloat(remainingBalance).toLocaleString('en-IN')}`, 380, currentY + 45, { width: 150 });
      
      currentY += 95;

      if (payment.notes) {
        doc.fontSize(10).font('Helvetica-Bold').fillColor(textColor).text('Notes / Remarks:', 50, currentY);
        doc.fontSize(9).font('Helvetica').fillColor(secondaryColor).text(`${payment.notes}`, 50, currentY + 14, { width: 495 });
        currentY += 45;
      }
      ...
      doc.font('Helvetica-Bold').fontSize(10).fillColor(textColor).text('THANK YOU FOR YOUR PAYMENT!', 50, currentY, { align: 'center', width: 495 });
      doc.font('Helvetica').fontSize(8).fillColor(secondaryColor).text('If you have any questions about this receipt, please contact our office.', 50, currentY + 14, { align: 'center', width: 495 });

      currentY += 50;

      // Signatures
      doc.font('Helvetica').fontSize(9).fillColor(textColor);
      doc.text('Authorized Signature', 350, currentY + 40, { align: 'center', width: 195 });
      doc.save();
      doc.rect(350, currentY + 35, 195, 0.5).fill(secondaryColor);
      doc.restore();
...
export async function generateOutstandingReportPDF({ patients, settings }) {
  const clinicSettings = settings || {
    clinic_name: 'Victoria Dental Care',
    tagline: 'Premium Dental Solutions',
    address: 'No 1/334 Injambakkam, Opp to Suga Jeeva Peralayam, Ammathi, Perumal Koil St, Chennai',
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Left: Clinic Info
      doc.fontSize(16).font('Helvetica-Bold').fillColor(textColor)
        .text(clinicSettings.clinic_name, 50, 30);
      doc.fontSize(9).font('Helvetica').fillColor(secondaryColor)
        .text('Outstanding Payments Report', 50, 48);

      // Right: Report Details
      doc.fontSize(9).font('Helvetica').fillColor(textColor);
      doc.text(`Date Generated: ${new Date().toISOString().split('T')[0]}`, 550, 30, { width: 241, align: 'right' });

      // Separator line
      doc.save();
      doc.rect(50, 65, 741, 1.5).fill(textColor);
      doc.restore();

      let currentY = 80;

      const drawReportRow = (y, patientName, phone, totalBilled, totalPaid, pending, isHeader = false) => {
        if (isHeader) {
          doc.font('Helvetica-Bold').fontSize(9).fillColor(textColor);
        } else {
          doc.font('Helvetica').fontSize(9).fillColor(textColor);
        }
        doc.text(patientName, 50, y, { width: 220 });
        doc.text(phone, 280, y, { width: 120 });
        doc.text(totalBilled, 410, y, { width: 100, align: 'right' });
        doc.text(totalPaid, 520, y, { width: 100, align: 'right' });
        doc.text(pending, 630, y, { width: 100, align: 'right' });
      };

      drawReportRow(currentY, 'PATIENT', 'PHONE', 'TOTAL BILLED (₹)', 'TOTAL PAID (₹)', 'PENDING (₹)', true);
      currentY += 15;
      ...
      let rowIndex = 0;
      patients.forEach(p => {
        const billed = parseFloat(p.totalBilled) || 0;
        ...
        if (rowIndex % 2 === 1) {
          doc.save();
          doc.rect(50, currentY - 3, 741, rowHeight).fill('#f8f9fa');
          doc.restore();
        }
      ...
      currentY += 5;
      doc.save();
      doc.rect(50, currentY, 741, 1.5).fill(textColor);
      doc.restore();
      currentY += 6;

      doc.font('Helvetica-Bold').fontSize(9.5).fillColor(textColor);
      doc.text('TOTALS', 50, currentY, { width: 220 });
      doc.text(grandBilled.toLocaleString('en-IN'), 410, currentY, { width: 100, align: 'right' });
      doc.text(grandPaid.toLocaleString('en-IN'), 520, currentY, { width: 100, align: 'right' });
      doc.text(grandPending.toLocaleString('en-IN'), 630, currentY, { width: 100, align: 'right' });
...
export async function generatePatientStatementPDF({ patient, ledger, settings }) {
  const clinicSettings = settings || {
    clinic_name: 'Victoria Dental Care',
    tagline: 'Premium Dental Solutions',
    address: 'No 1/334 Injambakkam, Opp to Suga Jeeva Peralayam, Ammathi, Perumal Koil St, Chennai',
  ...
  ledger.forEach(item => {
    if (item.type === 'Charge') totalBilled += parseFloat(item.amount) || 0;
    if (item.type === 'Credit') totalPaid += parseFloat(item.amount) || 0;
  });
  ...
  if (totalBilled > 0) {
    ...
    else if (totalPaid < totalBilled) paymentStatus = 'PARTIALLY PAID';
  }
  
  // Overdue status check if due date exists and is past
  const today = new Date().toISOString().split('T')[0];
  if (outstandingBalance > 0 && patient.due_date && today > patient.due_date) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Right: Statement Header
      doc.fontSize(18).font('Helvetica-Bold').fillColor(primaryColor)
        .text('ACCOUNT STATEMENT', 300, 40, { width: 245, align: 'right' });
      doc.fontSize(9).font('Helvetica').fillColor(textColor);
      doc.text(`Statement Date: ${today}`, 300, 65, { width: 245, align: 'right' });
      doc.text(`Account Status: ${paymentStatus}`, 300, 78, { width: 245, align: 'right' });
      if (patient.due_date) {
        doc.text(`Payment Due Date: ${patient.due_date}`, 300, 91, { width: 245, align: 'right' });
      }
      ...
      doc.text(`Name: ${patient.name}`, 50, currentY, { width: 240 });
      doc.text(`Age: ${patient.age || 'N/A'}`, 300, currentY, { width: 240 });
      currentY += 13;
      doc.text(`Phone: ${patient.phone || 'N/A'}`, 50, currentY, { width: 240 });
      doc.text(`Email: ${patient.email || 'N/A'}`, 300, currentY, { width: 240 });
      currentY += 13;
      if (patient.address) {
        doc.text(`Address: ${patient.address}`, 50, currentY, { width: 495 });
        currentY += 20;
      }
      ...
      const drawSummaryBox = (x, y, w, h, label, value, valueColor = textColor) => {
        ...
        doc.roundedRect(x, y, w, h, 6).lineWidth(0.5).strokeColor('#e9ecef').stroke();
        doc.restore();
        doc.font('Helvetica-Bold').fontSize(7).fillColor(secondaryColor).text(label, x + 10, y + 8, { width: w - 20 });
        doc.font('Helvetica-Bold').fontSize(10.5).fillColor(valueColor).text(value, x + 10, y + 20, { width: w - 20 });
      };

      drawSummaryBox(50, currentY, boxW, boxH, 'TOTAL BILLED CHARGES', `₹${totalBilled.toLocaleString('en-IN')}`);
      drawSummaryBox(218, currentY, boxW, boxH, 'TOTAL PAYMENTS RECEIVED', `₹${totalPaid.toLocaleString('en-IN')}`, '#34c759');
      drawSummaryBox(386, currentY, boxW, boxH, 'OUTSTANDING BALANCE', `₹${outstandingBalance.toLocaleString('en-IN')}`, outstandingBalance > 0 ? '#ff3b30' : textColor);
      currentY += boxH + 15;

      // Ledger Table Header
      doc.fontSize(11).font('Helvetica-Bold').fillColor(textColor).text('CHRONOLOGICAL LEDGER', 50, currentY);
      currentY += 16;
      ...
      const drawLedgerRow = (y, date, desc, type, charge, credit, balance, isHeader = false) => {
        if (isHeader) {
        ...
        doc.text(date, 50, y, { width: 60 });
        doc.text(desc, 115, y, { width: 145 });
        doc.text(type, 265, y, { width: 55 });
        doc.text(charge, 325, y, { width: 65, align: 'right' });
        doc.text(credit, 395, y, { width: 65, align: 'right' });
        doc.text(balance, 465, y, { width: 80, align: 'right' });
      };

      drawLedgerRow(currentY, 'DATE', 'TRANSACTION DETAIL', 'TYPE', 'CHARGE (Dr)', 'CREDIT (Cr)', 'BALANCE (₹)', true);
      currentY += 13;
      ...
      ledger.forEach(item => {
        ...
        if (rowIndex % 2 === 1) {
          doc.save();
          doc.rect(50, currentY - 3, 495, rowHeight).fill('#f8f9fa');
          doc.restore();
        }

        const dateStr = item.date;
        const descStr = item.description || '';
        const typeStr = item.type === 'Charge' ? 'Billed' : 'Payment';
        const chargeStr = item.type === 'Charge' ? parseFloat(item.amount).toLocaleString('en-IN') : '-';
        const creditStr = item.type === 'Credit' ? parseFloat(item.amount).toLocaleString('en-IN') : '-';
        const balanceStr = parseFloat(item.balance).toLocaleString('en-IN');

        drawLedgerRow(
          currentY,
          dateStr,
          descStr.substring(0, 32) + (descStr.length > 32 ? '...' : ''),
          typeStr,
          chargeStr,
          creditStr,
          balanceStr
      ...
      doc.font('Helvetica-Bold').fontSize(8.5).fillColor(textColor);
      doc.text('STATEMENT BALANCE DUE', 50, currentY);
      doc.text(`₹${outstandingBalance.toLocaleString('en-IN')}`, 465, currentY, { width: 80, align: 'right' });
