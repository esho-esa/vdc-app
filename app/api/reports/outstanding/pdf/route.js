...
import { generateOutstandingReportPDF } from '@/lib/pdf';
...
export async function GET(request) {
  try {
    ...
    // 5. Generate Landscape PDF Report
    const pdfBuffer = await generateOutstandingReportPDF({
      patients: outstandingPatients,
      settings: settings || undefined
    });

    const filename = `outstanding-payments-report-${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    console.error('[OutstandingPDF] Error:', error);
